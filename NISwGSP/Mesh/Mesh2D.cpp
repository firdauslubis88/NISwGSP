...
template <typename T>
int Mesh2D::getGridIndexOfPoint(const cv::Point_<T> & _p) const {
    cv::Point2i grid_p(_p.x / lw, _p.y / lh);
    grid_p.x = grid_p.x - (grid_p.x == nw);
    grid_p.y = grid_p.y - (grid_p.y == nh);
    return grid_p.x + grid_p.y * nw;
}

template int Mesh2D::getGridIndexOfPoint< float>(const cv::Point_< float> & _p) const;
template int Mesh2D::getGridIndexOfPoint<double>(const cv::Point_<double> & _p) const;
